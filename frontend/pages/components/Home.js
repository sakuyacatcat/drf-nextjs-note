...
const Home = (props) => {
    ...
    const { requestUser, toggleOnePostLike } = useContext(ApiContext);
    const { setViewMode } = useContext(ViewContext) || '';

    const showPosts = props.allPosts.sort(function (a, b) {
    ...
    const toggleLike = (event, postId) => {
        ...
        const evtClasses = event.currentTarget.classList
        const beLikeClass = classes.beLike

        if (evtClasses.contains(beLikeClass)) {
